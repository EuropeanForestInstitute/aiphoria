...
const edgeColors = ["rgba(59, 162, 114, 1)", "rgba(255, 50, 50, 1)"]
...
// Created when script is built
const yearToData = {year_to_data}
first_year = Object.keys(yearToData)[0]
const nodeData = yearToData[first_year]["node_index_to_data"]
const edgeData = yearToData[first_year]["edge_index_to_data"]

// Prebuilt mappings
const processIdToDataIndex = new Map()

let years = Object.keys(yearToData)
console.log("yearToData: ", yearToData)
...
for(const nodeIndex of Object.keys(nodeData)) {
  ...
  processIdToDataIndex.set(process_id, graphData.data.length)
...
for(const edgeIndex of Object.keys(edgeData)) {
  ...
  const newEdgeData = {
    ...
    lineStyle: {
      color: isUnitAbsoluteValue ? edgeColors[0] : edgeColors[1]
...
chart.on("timelinechanged", function (params) {
  console.log(params)
  const year = years[params.currentIndex]
  console.log(year)
})
